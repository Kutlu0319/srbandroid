...
def get_token(session: requests.Session, base_url: str, mac: str, timeout: int = 10) -> Optional[str]:
...
def get_channel_list(session: requests.Session, base_url: str, token: str, timeout: int = 10) -> Tuple[Optional[List[Dict]], Optional[Dict]]:
    headers = {"Authorization": f"Bearer {token}"}
    try:
